...
这里我们假设了添加接口会把被添加的对象返回给前端，当满足这个条件时，使用这个返回对象来生成一段描述。具体操作是：使用反射机制获取这个返回对象的每个字段及其对应值，使用各字段上的`Schema`注解的描述属性（它描述了字段含义） + 字段值 ，连接成一个字符串。当然如果你没有`swagger`或者类似的依赖，自定义一个注解也是完全可以的。