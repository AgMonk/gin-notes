...
- `maxLength`：最大输入长度
- `imeOptions`：设置为`actionSearch`时，可以让软键盘的回车键改为搜索键

Java

- `setOnFocusChangeListener`：设置焦点状态的变更监听
- `addTextChangedListener`：设置文本变化的监听
- `setOnEditorActionListener`：可以响应`imeOptions`中设置的动作，包括搜索
...
应用场景：我有一个`Activity`，这个`Activity`需要负责几种业务场景，每种业务场景有一定的相似性但也有很多不同，因此我希望根据业务场景从多个`Fragment`中选择一个装载到当前`Activity`中。



此时`Activity`不应该再有自己的布局，而是由装载的`Fragment`决定布局，因此在`onCreate`方法中不再需要执行`setContentView`方法。

然后我们需要在`onCreate`方法中手动创建`Fragment`，使用`setArguments`方法给`Fragment`传参，然后使用该方法装载`Fragment`
...
# 主题和样式(attr、style、theme)

## 属性(attr)

指的是xml标签上使用的属性，之前我们已经用过以`andriod:`或`app:`开头的属性，我们也可以自定义属性，自定义配置对所有标签都是有用的：

自定义属性定义在`res/values/attrs.xml`文件中(文件名固定)，如果不存在就手动创建，可以用`Android Studio`的`New - Values Resource File`创建，根节点为`<resources>`，子节点为`<attr/>` 它包含两个属性：

- name： 自定义的属性名
- formate：该属性可以接收的数据类型
...
<resources>
    <!--   列表中亮色背景 -->
    <attr name="colorRowLight" format="color" />
    <!--    列表中暗色背景-->
    <attr name="colorRowDark" format="color" />
    <!--    字体颜色-->
    <attr name="textColor" format="color" />
</resources>
```

定义好之后我们在布局文件中就可以通过代码提示看到可以给这些属性赋值了，但是光赋值还没用，变量被赋值之后还需要“被使用”，即对UI或行为产生影响。

如果希望把自定义属性的值复制给其他属性，可以用`?attr/自定义属性名`来引用它

## 风格(style)

使用风格可以对若干个属性(attr)进行赋值，xml标签应用风格之后就相当于使用了其中的赋值，类似于HTML中使用class统一配置css，可以让代码更简洁且易于维护。

风格定义在`res/values/styles.xml`文件中(文件名固定)，如果不存在就手动创建。文件根节点为`<resources>`

- 子节点为`<style>` ，使用属性`name`定义风格名称，使用`parent`属性指定继承其他风格的配置。
- `<style>`的子节点为`<item>`，`<item>`的`name`属性写属性的名称，节点内容写给属性的赋值。
...
<!-- 将自定义属性的值复制给其他属性 -->
<style name="popup_item_vertical" parent="wrap_content">
    <item name="android:textColor">?attr/colorRowDark</item>
    <item name="drawableTint">?attr/colorRowDark</item>
    <item name="android:layout_marginTop">12dp</item>
</style>
```

在标签中使用`style="@style/风格名称"`既可应用这套风格

## 主题(theme)

主题本质上就是风格，书写格式也是一样的，只是可以以`Activity`或整个APP为范围进行套用

- 为APP配置主题在`AndroidManifest.xml`文件的`android:theme`属性
- 为`Activity`配置主题需要在`setContentView`方法执行之前使用`setTheme`方法设置

主题定义在`res/values/themes.xml`中，注意还有套`夜间模式`的配置在`res/values-night/themes.xml`中，如果不希望跟随系统的夜间模式切换，需要把夜间模式继承的主题改为`.Light` 或`.Day`的，而不是带`.DayNight`之类的

对`Activity`配置属性时，可以使用如下方法判断当前系统是否处在夜间模式下，可以自行决定是否使用夜间主题
...
```xml
<style name="Theme.Nga" parent="Base.Theme.Nogayallianceforandroid">
    <item name="android:textColor">?attr/textColor</item>
</style>

<style name="Theme.Nga.Yellow" parent="Theme.Nga">
    <item name="textColor">@color/yellow_text_color</item>
    <item name="colorRowLight">@color/yellow_light</item>
    <item name="colorRowDark">@color/yellow_dark</item>
</style>

<style name="Theme.Nga.Green" parent="Theme.Nga">
    <item name="textColor">@color/green_text_color</item>
    <item name="colorRowLight">@color/green_light</item>
    <item name="colorRowDark">@color/green_dark</item>
</style>

<style name="Theme.Nga.Black" parent="Theme.Nga">
    <item name="textColor">@color/black_text_color</item>
    <item name="colorRowLight">@color/black_light</item>
    <item name="colorRowDark">@color/black_dark</item>
</style>
```

这里的基类主题把最常用的`android:textColor`的默认字体颜色改为由自定义属性来决定

然后在三个子主题中分别规定了自定义属性的值
...
在`Activity`中重写方法，`inflate`方法可以执行多次，会依次添加菜单项
...
另外还可以重写`onPrepareOptionsMenu`方法，该方法在每一次展开菜单时都会调用，可以用来修改菜单项的可见性等。
...
不创建配置类也可以正常使用，但控制台会弹警告，简单地创建一个类即可。
...
另外可以配置`Glide`使用已有的`OkhttpClient`执行网络请求
...
public class MyAppGlideModule extends AppGlideModule {
    ...
    public void registerComponents(@NonNull Context context, @NonNull Glide glide, @NonNull Registry registry) {
        ...
        registry.replace(GlideUrl.class, InputStream.class, new OkHttpUrlLoader.Factory((Call.Factory) MyApplication.app.okHttpClient));
    }
}
```
...
public class ClipboardUtils {
     ...
     * @return 文本
     */
    public static String getText(Activity activity, @IntRange(from = 0) int index) {
        ClipboardManager clipboard = getClipboardManager(activity);
        if (clipboard.hasPrimaryClip()) {
            ClipData clipData = clipboard.getPrimaryClip();
            if (clipData != null && clipData.getItemCount() > index) {
                return clipData.getItemAt(index).getText().toString();
            }
        }
        return null;
    }
